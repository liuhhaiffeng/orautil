...
inline ASPConnection::ASPConnection(const ocilib::Connection& con)
	: m_pCon(new ocilib::Connection()),
	  m_pSt(new ocilib::Statement())
{
	ocilib::Connection* pTmpCon = new ocilib::Connection(con);
	ocilib::Statement* pTmpSt = new ocilib::Statement(con);
	Release();
	m_pCon = pTmpCon;
	m_pSt = pTmpSt;	
}
	  ...
	  m_pSt(new ocilib::Statement())
{
	if (this != &con)
	{
		ocilib::Connection* pTmpCon = new ocilib::Connection(*(con.m_pCon));
		ocilib::Statement* pTmpSt = new ocilib::Statement(*(con.m_pCon));
		Release();
		m_pCon = pTmpCon;
		m_pSt = pTmpSt;	
	}
...
inline void ASPConnection::Prepare(const std::string& sql)
{
	m_pSt->Prepare(sql);
}

template <typename TDataType>
inline void ASPConnection::BindIn(const std::string& name, TDataType& value)
{
	// 绑定需要传递并保存value的引用, 并且需要在执行ExecutePrepare()是, 此引用要有效
	// 但ASPDate无法支持ocilib.Bind(), 下面调用会出错, 故绑定日期只能要求使用字符串
	// 形式, 如: to_date('strDate', 'strFormat');
	/*ocilib::Date dt(value.ToString("yyyy-mm-dd hh24:mi:ss"), "yyyy-mm-dd hh24:mi:ss");
	m_pSt->Bind(name, dt, ocilib::BindInfo::In);*/

	static_assert(!std::is_same<TDataType, ASPDate>::value, 
		"BindIn not support ASPDate type, please use string-date. ex: to_date('strDate', 'strFormat')");

	static_assert(!std::is_same<TDataType, std::string>::value,
		"std::string type, please use BindIn(const std::string& name, std::string& value, unsigned int maxSize) function");

	m_pSt->Bind(name, value, ocilib::BindInfo::In);
}

inline void ASPConnection::BindIn(const std::string& name, std::string& value, unsigned int maxSize)
{
	m_pSt->Bind(name, value, maxSize, ocilib::BindInfo::In);
}

inline void ASPConnection::ExecutePrepared()
{
	m_pSt->ExecutePrepared();
}
