...
#endif
{
	...
	while (true)
	{
		...
		string jlbh(szJLBH.data());
		time_t tt = time(NULL);		
		string strXrsj = ASPDate::Time2Str(tt, "yyyy-mm-dd hh24:mi:ss");
		
		try
		{
			ASPConnection con = aspPool.GetConnection();
			con.Prepare("insert into b_mutltest(jlbh, xrsj, threadidx) values (:jlbh, to_date(:xrsj, 'yyyy-mm-dd hh24:mi:ss'), :threadidx)");
			con.BindIn(":jlbh", jlbh, static_cast<unsigned int>(jlbh.size()));
			con.BindIn(":xrsj", strXrsj, static_cast<unsigned int>(strXrsj.size()));
			con.BindIn(":threadidx", id);
			con.ExecutePrepared();
			con.Commit();
			con.Close();
		}
