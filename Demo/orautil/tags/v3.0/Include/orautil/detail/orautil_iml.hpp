...
inline bool IsConnected(const AutoConnection& con)
{
	return con->IsServerAlive();
}
...
template<>
ocilib::Date SelectOne<ocilib::Date>(const AutoConnection& con, const ocilib::ostring& sql)
{
	ocilib::Statement st(*con.get());
	st.Execute(sql);
	ocilib::Resultset rs = st.GetResultset();
...
inline void ConnectionDeleter(ocilib::Connection* pCon)
{
	if (pCon)
	{
		if (!pCon->IsNull())
...
inline void PoolDeleter(ocilib::Pool* pPool)
{
	if (pPool)
	{
		if (!pPool->IsNull())
